...
# 用法：
# pwd: XFileTransit
# bash ./tools/package/generate_release_tar_gz.sh ./build-Linux-Debug/src/XFileTransit /home/xqqyt/Qt6.8/6.8.3/gcc_64

EXECUTABLE="$1"
QT_GCC_PATH="$2"

PACKAGE_DIR="/tmp/XFileTransit"
...
cp "src/res/logo/logo_small.png" "$PACKAGE_DIR/logo.png"

# 拷贝构建好的可执行文件
cp "$EXECUTABLE" "$PACKAGE_DIR/XFileTransit"

echo "Files prepared in: $PACKAGE_DIR"

# 生成 tar.gz 压缩包
ARCHIVE_NAME="XFileTransit_$(date +%Y%m%d_%H%M%S).tar.gz"
ARCHIVE_DIR="$(dirname "$PACKAGE_DIR")/XFileTransit-tar-gz"

mkdir -p "$ARCHIVE_DIR"

echo "Creating archive: $ARCHIVE_NAME ..."
tar -czf "$ARCHIVE_DIR/$ARCHIVE_NAME" -C "$PACKAGE_DIR" $(ls "$PACKAGE_DIR")
