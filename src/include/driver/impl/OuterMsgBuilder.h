...
public:
    ...
    std::unique_ptr<OuterMsgBuilderInterface::UserMsg> buildMsg(std::string payload) override;
    std::unique_ptr<OuterMsgBuilderInterface::UserMsg> buildMsg(std::vector<uint8_t> payload) override;
private:
    std::unique_ptr<OuterMsgBuilderInterface::UserMsg> build(std::vector<uint8_t> payload) override;
    uint8_t version;
};
